...
peakCallingJob <- function( IDsec, IDpeak, peak_calling=TRUE, annotation=TRUE ) {
	...
	REFGENOME <- unique( unlist( sapply( IDpreList$primary_id, function(x) extractInfoFromDB( paste0('SELECT genome FROM pa_options, primary_analysis WHERE pa_options.id = options_id AND source = origin AND primary_analysis.id=',x ) ) ) ) )
