...
# counts_total_introns
			...
			saveRDS( sythesis, paste0( exp_name,"-synthesis.rds" ) )
			saveRDS( degradation, paste0( exp_name,"-degradation.rds" ) )
			saveRDS( processing, paste0( exp_name,"-processing.rds" ) )
			saveRDS( pre_mRNA, paste0( exp_name,"-pre_mRNA.rds" ) )
			saveRDS( total_mRNA, paste0( exp_name,"-total_mRNA..rds" ) )
...
# For eac row, add in the good condition one
...
sapply(1:length(values$primary_id), function(x) if (values$cond[x] == condition1) { 
	bamfiles_4sU_cond1 <- c(bamfiles_4sU_cond1, values$primary_id[x]); 
	bamfiles_RNAtotal_cond1 <- c(bamfiles_RNAtotal_cond1, values$rnatotal_id[x]); 
} else  {
	bamfiles_4sU_cond2 <- c(bamfiles_4sU_cond2, values$primary_id[x]); 
	bamfiles_RNAtotal_cond2 <- c(bamfiles_RNAtotal_cond2, values$rnatotal_id[x]); 
})
...
rpkms_cond1_foursu_exons <- rpkms$foursu_exons[exon_filt,,drop=FALSE]
rpkms_cond1_total_exons <- rpkms$total_exons[exon_filt,,drop=FALSE]
rpkms_cond1_foursu_introns <- rpkms$foursu_introns[intron_filt,,drop=FALSE]
rpkms_cond1_total_introns <- rpkms$total_introns[intron_filt,,drop=FALSE]

counts_cond1_foursu_exons <- counts$foursu$exonCounts[exon_filt,,drop=FALSE]
counts_cond1_total_exons <- counts$total$exonCounts[exon_filt,,drop=FALSE]
counts_cond1_foursu_introns <- counts$foursu$intronCounts[intron_filt,,drop=FALSE]
counts_cond1_total_introns <- counts$total$intronCounts[intron_filt,,drop=FALSE]

# cond 2

rpkms_cond2_foursu_exons <- rpkms$foursu_exons[exon_filt,,drop=FALSE]
rpkms_cond2_total_exons <- rpkms$total_exons[exon_filt,,drop=FALSE]
rpkms_cond2_foursu_introns <- rpkms$foursu_introns[intron_filt,,drop=FALSE]
rpkms_cond2_total_introns <- rpkms$total_introns[intron_filt,,drop=FALSE]

counts_cond2_foursu_exons <- counts$foursu$exonCounts[exon_filt,,drop=FALSE]
counts_cond2_total_exons <- counts$total$exonCounts[exon_filt,,drop=FALSE]
counts_cond2_foursu_introns <- counts$foursu$intronCounts[intron_filt,,drop=FALSE]
counts_cond2_total_introns <- counts$total$intronCounts[intron_filt,,drop=FALSE]
