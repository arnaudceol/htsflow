...
samplesAll=c(15693,15694,15695, 15699,15700,15701, 15702,15703,15704, 15705,15706,15707, 15696,15697,15698, 15734,
		15728,15729,15730, 15735,15736,15737, 15738,15739,15740, 15741,15742,15743, 15731,15732,15733)
snamesAll=c('S_0h_wt_1','S_0h_wt_2','S_0h_wt_3', 'S_2h_wt_1','S_2h_wt_2','S_2h_wt_3',
...
lsizeAll=readRDS(paste0(dirout, 'lsize.rds'))


samples=samplesAll[1:numSamples]
snames=snamesAll[1:numSamples]
lsize=lsizeAll[1:numSamples]
...
colnames(counts)=snames #snames[,2]
...
for(i in 1:length(samples))  RPKM[,i] = RPKM[,i] / as.numeric(lsize)[i]
